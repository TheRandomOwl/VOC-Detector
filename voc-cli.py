...
@click.pass_context
def cli(ctx, cache, smoothness, fft, y_offset):
    """A CLI tool to analyze data from Picoscope 7."""
    # Store options in context
    ctx.ensure_object(dict)
    ctx.obj.update(cache=cache, smoothness=smoothness, fft=fft, y_offset=y_offset)
...
def compare(ctx, folder_a, folder_b, save_dir):
    ...
    run_a = voc.Run(folder_a, cache=ctx.obj['cache'], smoothness=ctx.obj['smoothness'], y_offset=ctx.obj['y_offset'])
    run_b = voc.Run(folder_b, cache=ctx.obj['cache'], smoothness=ctx.obj['smoothness'], y_offset=ctx.obj['y_offset'])
    voc.plot_average_signals(run_a, run_b, save_path, fft=ctx.obj['fft'], show=show_plot)
